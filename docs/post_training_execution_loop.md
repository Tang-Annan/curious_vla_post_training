...
> 状态更新（2026-08-24）：本文已冻结为旧 `G=2` GRPO 历史证据。用户明确开启的新 `G=4` reward/selector 路线由 [`grpo_g4_experiment_ledger.md`](grpo_g4_experiment_ledger.md) 记录；本文件的既有完成记录与负结果不回写。
>
> 在 2026-08-14 的旧 GRPO 收尾阶段，本文档是当时唯一执行方案与决策台账。服务器实验目录中的日志、配置和产物是原始证据，本文档保存可追溯事实、解释边界、阶段门控和下一步决策。已完成记录只追加、不回写；外部论文只提供待验证假设，不能替代 Curious-VLA 自身证据。
