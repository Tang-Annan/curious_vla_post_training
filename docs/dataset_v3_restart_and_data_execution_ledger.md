...
> 当前状态：`S1_COMPLETE / R0_COMPLETE / H0_COMPLETE / M0_COMPLETE / E0_COMPLETE / RR_COMPLETE_GATE_CLOSED / TC_COMPLETE_GATE_CLOSED / TR_COMPLETE_GATE_CLOSED / RC_SKIPPED_BY_USER / TC_PPO2_COMPLETE_GATE_CLOSED / TAIL_SEMANTIC_AUDIT_COMPLETE / TRAIN_EVAL_TAIL_MISALIGNED / FINAL_UNACCESSED`；保留现有 `models/sft_stage2`，将 118 个 SFT-unseen logs / 835 个 eligible scenes 全部保留给 Dev/Final，并只从 1,192 个 SFT-seen logs / 103,288 个 SFT tokens 构建受控、可审计的 GRPO train-side pool；Random/TailMix manifests、CDT `R_task`、原 PPO-epoch-1 配置、M0 协议及 SFT Dev anchor 均已冻结；RR/TC/TR discovery 均未通过晋级门槛；2026-08-31 用户决定跳过 RC、终止完整 2×2 interaction 路线，并将最后一次方法尝试切换为从 SFT Stage-2 重启的 `TailMix + CDT + ppo_epochs=2`；multi-epoch iterator 修复、epoch-indexed PPO telemetry、focused test 与 2-step smoke 均已通过，但正式训练的 monitor 与严格 unseen Dev 均退化，最后一次尝试按冻结停止规则关闭；后续 CPU-only 语义审计确认训练侧 policy-conditioned Tail 与评价侧 GT actor-proximity Tail 在全部预注册方向检查上错位。
...
### 记录 V3-022：训练 Tail 与评价 Tail 的 CPU-only 语义对齐审计

- 时间与资源：正式 retry1 于 2026-08-31 15:14:54–15:17:58（Asia/Shanghai）完成，墙钟 `184 s`；服务器为无卡模式，cgroup `cpu.max=50000 100000`（0.5 vCPU）、`memory.max=2,147,483,648` bytes，固定 `workers=1` 且 `CUDA_VISIBLE_DEVICES` 为空；运行期间文件页缓存使 cgroup memory 接近上限，但 Python RSS 约 258 MiB，终态 `oom=0`、`oom_kill=0`；
- 启动与源码证据：审计实现 commit=`95d52bee03dd13fa2e1855a9b3e4d91b38eae5a4`；首次启动在创建 run 目录和读取原始日志前即因 Bash 特殊变量 `$RANDOM` 与 manifest 路径赋值冲突而退出，原始失败日志 `/root/autodl-tmp/v3-tail-semantic-audit-launch.log` 保留；仅将该变量改名为 `RANDOM_MANIFEST` 的修复 commit=`08e62f747f0e1f8c44e7f0cfb5bfaddb2d980e20`，bundle 经本地/服务器 verify 后 fast-forward；正式 run 的 `source_commit.txt` 与远端 HEAD 均为 `08e62f7`，启动与终态 tracked source clean；
- 输入与隔离边界：只使用 SFT-seen Screen 8,000 scenes、冻结 Random/TailMix 各 2,000 scenes、S1 stability/capacity 表，以及既有严格 SFT-unseen Dev Natural 210 / Tail 206 和 SFT/RR/TC/TR/TC-PPO2 五组 scene metrics；Master Index 只按 Dev token 物化场景属性，未读取 Final-specific manifest/parquet/metric/outcome；`run.env` 与报告均记录 `dev_accessed=true`、`final_accessed=false`，运行目录无 Final 产物；统计固定为按 `log_name` 聚类的 20,000 次 bootstrap，seed=`20260831`；
- 定义审计：训练 TailMix 是“policy-conditioned dual-rollout stability categories + matched random anchors”，评价 Tail 是“policy-independent log-level GT actor proximity enrichment（vehicle `<=5 m` 或 VRU `<=10 m`）”，两者定义不相同；冻结 Dev manifest 上 Natural/Tail 的 GT interaction rate 分别为 `100/210=47.619%` 与 `165/206=80.097%`，说明评价 Tail 确实富集近距离交互；
- 训练 selector 对齐结果：Random/TailMix 的 GT interaction rate 为 `33.10%/31.25%`，TailMix−Random=`-1.85 pp`，log-cluster bootstrap CI=`[-4.364,+0.636] pp`，没有 GT interaction enrichment；更直接的 stable policy-risk 653 scenes 与 random-anchor 7,347 scenes 的 interaction rate 为 `24.655%/33.864%`，差值=`-9.209 pp`，CI=`[-13.015,-5.356] pp`，即 policy-conditioned 风险类别反而显著减少评价 Tail 所强调的近距离交互；
- train→Dev support：interaction-rate absolute gap 为 Random→Dev-Tail `46.997 pp`、TailMix→Dev-Tail `48.847 pp`；proximity-bin JSD 为 `0.192982/0.205891`，TailMix 更远；map-location JSD 为 `0.171409/0.207290`，month JSD 为 `0.181704/0.217645`，同样没有更接近 Dev Tail；六项 directional checks 全为 false，机械状态=`MISALIGNED_ON_ALL_DIRECTIONAL_CHECKS`；
- policy-level 关联：GT interaction scenes 对五个模型都更难；全 Dev 的 interaction−noninteraction StrictClear 差值范围为 `-11.09` 至 `-13.45 pp`，在 Dev Tail 内为 `-12.64` 至 `-14.46 pp`，说明评价 Tail 的近距离交互语义能够稳定定位 safety failure；但 Tail StrictClear 仅为 SFT/RR/TC/TR/TC-PPO2=`74.27%/74.76%/75.24%/74.27%/73.79%`，TC 的小幅改善未形成稳定晋级，PPO epoch 2 也未修复 support 错位；该关联是诊断证据，不作 selector→policy outcome 的因果估计；
- 产物与验收：run=`semantic_audit/v3_tail_semantic_alignment_20260831`，`COMPLETE/exit_code=0`、无 `RUNNING/FAILED`；`train_scene_features.csv` 为 8,000 data rows，SHA-256 `a6a6e9dfb6e68baf25ae848b14871df996684269f665e1359c175eb16b7c7cad`；`dev_model_outcomes.csv` 为 2,080 data rows，SHA-256 `3c23f219a065cfc991936719b930fa69c8d2a29ec47a976dfed45c838373acdb`；`tail_semantic_alignment_report.json` SHA-256 `a961194a83697aa8d6e9c14876187875f88d1d410fff866a6418bafc5de38f20`；全部 input/result hashes 复验通过；数据盘终态可用 `38,749,925,376` bytes；
- 结论边界：本审计确认“训练 Tail 与评价 Tail 存在结构性且数据上可见的语义错位”。现有 TailMix/CDT 改善的是 policy-conditioned reward/advantage geometry，而没有增加 Dev Tail 所需的 GT actor-proximity support；因此 selector 目标错位与此前观测到的小 KL、弱 clipping、有限 policy update 可以同时成立。该结论不要求重开已关闭的 V3 训练路线，也不授权访问 Final；若未来启动新一轮实验，应先把 policy-failure 与 GT-interaction 拆成独立分层或二维配额，再重新冻结训练和评价语义。
