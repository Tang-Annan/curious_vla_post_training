...
RUN_DIR="$EXPERIMENT_ROOT/semantic_audit/v4_experiment_closure_cpu_20260831"

for path in "$PYTHON" "$CAPACITY/COMPLETE" "$DECISION/COMPLETE" "$V3_AUDIT/COMPLETE" \
    "$TRAIN_SCENES" "$TRAIN_TIERS" "$DEV_TIERS" "$DEV_MODELS"; do
...
printf 'run_id=v4_experiment_closure_cpu_20260831\ncuda_visible_devices=empty\nbootstrap_resamples=20000\ndev_accessed=true\nfinal_accessed=false\ngpu_training_authorized=pending_cpu_gate\n' > "$RUN_DIR/run.env"
sha256sum "$TRAIN_SCENES" "$TRAIN_TIERS" "$DEV_TIERS" "$DEV_MODELS" > "$RUN_DIR/input_sha256.txt"
