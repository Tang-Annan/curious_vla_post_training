...
VAL_FILTER_FILE="${VAL_FILTER_FILE:-$WORKSPACE_ROOT/manifests/dev_tokens.txt}"
...
for path in "$MODEL_PATH" "$DATA_PATH" "$FILTER_FILE" "$VAL_FILTER_FILE" "$CACHE_PATH/metadata"; do
...
"$WORKSPACE_ROOT/envs/curious/bin/python" -m verl.trainer.main \
    ...
    data.val_token_filter_file="$VAL_FILTER_FILE" \
