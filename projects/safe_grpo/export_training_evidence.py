...
def export_samples(rollouts_path: Path, output_path: Path) -> dict[str, Any]:
    ...
    rows = [row for row in all_rows if row.get("evidence_phase", "train") == "train"]
    if not rows:
        raise ValueError("Training rollout evidence is empty.")
    ...
    return {
        ...
        "raw_response_available": any(
            "raw_response" in row or "response" in row for row in rows
        ),
